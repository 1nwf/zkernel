...
pub fn init() void {
    ...
    timer.init_timer(20);
    idt.idt = [_]idt.Entry{idt.Entry.init(@ptrToInt(&isr.isr_common))} ** 256;
    initExceptions();
    initInterrupts();
    idt.descriptor = idt.IDTDescr.init(@sizeOf(idt.IDT) - 1, @ptrToInt(&idt.idt));
    idt.descriptor.load();
    enable();
}
...
pub fn initExceptions() void {
    ...
    for (fns, 0..) |handler, i| {
        idt.setHandler(@truncate(u8, i), @ptrToInt(handler));
    }
}
